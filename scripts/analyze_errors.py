#!/usr/bin/env python3
"""
Analyze errors from NLI model predictions on ANLI and other evaluation sets.
...
def main():
    ...
    parser.add_argument('--snli', type=str,
                        default='eval_output_snli/eval_predictions.jsonl',
                        help='Path to SNLI evaluation predictions (optional)')
    ...
    # Load SNLI errors if file provided and exists
    if args.snli and Path(args.snli).exists():
        print(f"Loading SNLI predictions from {args.snli}")
        snli_preds = load_predictions(args.snli)
        snli_errors = extract_errors(snli_preds, 'SNLI')
        print(f"Found {len(snli_errors)} errors in SNLI")

        if args.max_errors_per_source:
            snli_errors = snli_errors[:args.max_errors_per_source]
        all_errors.extend(snli_errors)
